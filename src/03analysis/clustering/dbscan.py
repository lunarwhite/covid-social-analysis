...
def preProcess(df):
    ...
    df['content_cut'] = df['content_cut'].map(lambda x: stop_words_cut(x, 'res\\input\\dict\\stop-words.txt'))
...
def feature_extraction(series, vec_args):
    vec_args_list = ['%s=%s' % (i[0], "'%s'" % i[1] if isinstance(i[1], str) else i[1]) for i in vec_args.items()]
...
if __name__ == '__main__':
    filepath = 'res\\input\\topic\\疫情1.20.csv'
    ...
    matrix = feature_extraction(df['content_'], vec_args={'max_df': 0.95, 'min_df': 1, 'max_features': max_features})
    eps_var = 0.05
    min_samples_var = 5
    ...
    # plt.savefig('dbscan-example.jpg')
    plt.show()
    ...
    yticks1 = [str(get_most_common_words(df_non_outliers[df_non_outliers['rank'] == i]['content_cut'], top_n=10)) + str(i) for i in range(1, rank_num + 1)]
