...
def words():
    senList = readLines2('res\\input\\dict\\BosonNLP-sentiment-score.txt')
    senDict = defaultdict()
    for s in senList:
        senDict[s.split(' ')[0]] = s.split(' ')[1]

    exList = readLines2('res\\input\\dict\\extra.txt')
    exDict = defaultdict()
    for x in exList:
        exDict[x.split(' ')[0]] = x.split(' ')[1]

    notList = readLines2('res\\input\\dict\\notDict.txt')
    degreeList = readLines2('res\\input\\dict\\degreeDict.txt')
...
if __name__ == "__main__":
    # filepwd = eachFile("res\\input\\comment")
    # score_var = []
    # words_vaule = words()
    # for x in filepwd:
    ...
    #         i = i + 1
    #     i = 0
    #     text_save(x.replace('res\\input\\comment', 'res\\output\\comment-txt'), score_var)
        # score_var = []

    files = eachFile('res\\output\\topic-txt')
    file = open('res\\output\\emotion-dict\\result.txt', 'w+', encoding='utf-8')
    for x in files:
    ...
    file = open('res\\output\\emotion-dict\\result.txt', 'r', encoding='utf-8')
    l = file.read().split('\n')
    file2 = open('res\\output\\emotion-dict\\差值.txt', 'w+', encoding='utf-8')
    for x in l:
        ls = x.split('  ')
